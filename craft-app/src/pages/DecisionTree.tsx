  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const fetchedRoot = data.root as TreeNode;
        // Defensive check: only accept it if it has the shape we expect.
        if (fetchedRoot && typeof fetchedRoot === 'object' && Array.isArray(fetchedRoot.children)) {
          setTitle(data.title ?? '');
          setRoot(fetchedRoot);
          setRowId(data.id);
        } else {
          setLoadError('This decision\'s data looks corrupted and could not be loaded.');
        }
      } catch (err: any) {
